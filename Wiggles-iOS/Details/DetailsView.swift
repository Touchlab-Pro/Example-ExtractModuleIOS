                ...
                ScrollView(.vertical, showsIndicators: false) {
                    VStack {
                        ZStack(alignment: .top) {
                            Image(lookupImage(image: viewModel.model.image)).resizable()
...
struct DetailsOwnerView: View {
    var image: SharedPuppyDB.Image, name: String, bio: String
    var messageMethod: () -> ()
    var body: some View {
        HStack {
            Image(lookupImage(image: image)).resizable().scaledToFill().frame(width: 60, height: 60).cornerRadius(30)
...
struct DetailsView_Previews: PreviewProvider {
    static var previews: some View {
        Group {
            DetailsView(model: FakeDogDatabase.shared.dogList[0])
        }
    }
}
